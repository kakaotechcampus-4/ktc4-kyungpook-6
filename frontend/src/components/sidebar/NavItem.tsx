...
function NavItem({ icon, label, active = false, className, ...props }: NavItemProps) {
  return (
    <button
      type="button"
      aria-current={active ? 'page' : undefined}
      className={[
        'flex w-full flex-col items-center justify-center gap-1 overflow-clip p-3',
        active
          ? 'rounded-lg bg-[#fefce8] text-[#eab308]'
          : 'rounded-[10px] text-[#475569]',
        className,
      ]
        .filter(Boolean)
        .join(' ')}
