...
public enum NtsLookupResult {
    ...
    NO_BIZ_NO;   // 사업자번호가 없거나 형식이 틀려 조회하지 않음 — 번호부터 찾거나 바로잡아야 함

    /**
     * 우리 DB 의 사업자등록번호가 잘못됐다고 볼 수 있는가 — 번호부터 찾거나 바로잡아야 하는 건이다.
     *
     * <p>{@link #UNCONFIRMED} 는 다시 조회하면 확인될 수 있어 데이터 문제로 보지 않는다.
     * 번호가 있는데 국세청에 없는 경우(NOT_REGISTERED)와 합쳐 응답의 {@code dataProblem} 이 된다.
     */
